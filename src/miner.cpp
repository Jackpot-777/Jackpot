...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("JackpotMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("JackpotMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("JackpotMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("jackpot-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running JackpotMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
