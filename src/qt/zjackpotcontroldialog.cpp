...
#include "ui_zjackpotcontroldialog.h"
...
std::list<std::string> ZJackpotControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZJackpotControlDialog::listMints;

ZJackpotControlDialog::ZJackpotControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZJackpotControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZJackpotControlDialog::~ZJackpotControlDialog()
{
    delete ui;
}

void ZJackpotControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZJackpotControlDialog::updateList()
{
...
void ZJackpotControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZJackpotControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZJackpot_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZJackpotControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZJackpotControlDialog::GetSelectedMints()
{
...
void ZJackpotControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
