...
#include "zjackpotcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 z777 ought to be enough for anybody." - Bill Gates, 2017
    ui->z777payAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelz777SyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintz777->setEnabled(false);
        ui->pushButtonMintz777->setToolTip(tr("z777 is currently disabled due to maintenance."));

        ui->pushButtonSpendz777->setEnabled(false);
        ui->pushButtonSpendz777->setToolTip(tr("z777 is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->z777payAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintz777_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("z777 is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " z777...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" z777 in ") +
...
void PrivacyDialog::on_pushButtonSpendz777_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("z777 is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn z777
        sendz777();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send z777
    sendz777();
}

void PrivacyDialog::on_pushButtonZJackpotControl_clicked()
{
    ZJackpotControlDialog* z777Control = new ZJackpotControlDialog(this);
    z777Control->setModel(walletModel);
    z777Control->exec();
}

void PrivacyDialog::setZJackpotControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelz777Selected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendz777()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Jackpot Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->z777payAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as z777 is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " JACKPOT </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " z777</b>";
    ...
    // use mints from z777 selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZJackpotControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZJackpotControlDialog::GetSelectedMints();
    }
    ...
    // Clear zjackpot selector in case it was used
    ZJackpotControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("z777 Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Jackpot, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("z777 Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " z777 </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" z777 "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" z777 "));
    ui->labelz777AmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelz777SyncStatus->setVisible(fShow);
}
